...
import { identifyPlant, type IdentificationResult } from '@/services/plantnet';
import { addUserPlant } from '@/services/user';
...
export default function Scan() {
  ...
  const runIdentify = async () => {
    try {
      ...
      const identifications = await identifyPlant(preview, 'leaf', apiKey);
      
      // Show top 5 results
      setResults(identifications.slice(0, 5));
    } catch (e: any) {
  ...
  const savePlantToInventory = async (result: IdentificationResult) => {
    if (!user) {
      Alert.alert('Authentication required', 'Please sign in to save plants to your inventory.');
      return;
    }

    try {
      setSaving(true);
      await addUserPlant(user.uid, result.label);

      Alert.alert('Success', 'Plant added to your inventory!');
      setPreview(null);
      setResults(null);
    } catch (error: any) {
      const message = error?.message || 'Failed to add plant to inventory.';
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Add',
          onPress: () => {
            void savePlantToInventory(result);
          },
        },
      ]
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <LoadingOverlay
        ...
        message={loading ? 'Identifying plant...' : 'Saving plant...'}
      />
    </SafeAreaView>
  );
}
